...
#### Logger
- ✅ 删除 `services/infra/logger/` 模块，使用新的日志系统 (lazyLoggers)
- ✅ 所有基础设施层代码移除 traceId 使用

#### Database
- ✅ 统一数据库系统：仅保留 `database/` 目录
- ✅ 删除重复的 `services/infra/database/` 系统
- ✅ 所有DAO文件迁移至 `database/dao/*`
- ✅ 所有导入路径已更新（~40 个文件）
...
| 模块 | 迁移前路径 | 迁移后路径 | 文件数 |
|------|-----------|-----------|--------|
| Logger | `utils/logger.js` | `lazyLoggers` (新日志系统) | ~150 |
