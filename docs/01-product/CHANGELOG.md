...
### 新增
- ✅ Layer 6 工作流编排引擎：可视化编辑器、YAML/JSON 解析、断点调试
- ✅ Layer 5 执行引擎增强：任务调度器、结果收集器、消息处理器
- ✅ 工作流 IPC API：`workflow:save`、`workflow:run`
- 添加网格预览功能，支持实时截图更新
...
### 优化
- ✅ 更新架构文档：修正 Layer 6 实现状态（从"待实现"改为"已实现"）
- ✅ 更新目录结构说明：反映实际的代码组织（business/、services/、features/）
- ✅ 更新执行引擎模块文档：反映实际的服务结构（ExecutionManager、TaskScheduler、ResultCollector）
- ✅ 更新 IPC API 文档：添加工作流相关的 IPC 接口（workflow:save、workflow:run）
